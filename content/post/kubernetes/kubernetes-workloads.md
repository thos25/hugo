...
Consistent UID/GID across every app made the NFS permissions story from the [storage post]({{< ref "kubernetes-storage" >}}) a lot simpler: one user, one group, every app writing to shared volumes without stepping on each other's file ownership.
