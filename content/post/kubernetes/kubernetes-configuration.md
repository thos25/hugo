...
**Stage 3 — Sealed Secrets.** This is where I landed, and where new secrets go today. The [Bitnami Sealed Secrets](https://github.com/bitnami-labs/sealed-secrets) controller lets you encrypt a Secret client-side with its public key, commit the encrypted `SealedSecret` to git, and only the controller running in-cluster can decrypt it back into a real Secret. The workflow, including the PowerShell-specific piping since I do this from Windows:
